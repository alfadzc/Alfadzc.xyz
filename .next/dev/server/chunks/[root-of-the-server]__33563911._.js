...
"[project]/app/api/validators/lava/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET() {
    try {
        const RPC_URL = "https://lava-api.polkachu.com";
        const VALIDATOR_OPERATOR = "lava@valoper18kuml80whhgw3g0dp2qthtlcsazvekfd9kzttd";
        const CHAIN_DIVISOR = 1_000_000;
        const PRICE_USD = 0.03550907;
        // Get ONLY bonded validators
        const validatorRes = await fetch(`${RPC_URL}/cosmos/staking/v1beta1/validators?status=BOND_STATUS_BONDED&pagination.limit=500`);
        const validatorData = await validatorRes.json();
        const validators = validatorData.validators || [];
        // Find my validator
        const myValidator = validators.find((v)=>v.operator_address === VALIDATOR_OPERATOR);
        if (!myValidator) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__33563911._.js.map