...
const LCD_URL = "https://api-test.monolyth.vinjan-inc.com";
const VALIDATOR_OPERATOR = "monovaloper10ers0hza3hg8nwy37rtcn9svje05md53uf7hdl";
const CHAIN_DIVISOR = 1_000_000_000_000_000_000;
const PRICE = 0;
const FALLBACK = { chain: "Monolythium", moniker: "alfadzc", operatorAddress: VALIDATOR_OPERATOR, totalBonded: "0", totalBondedUSD: "0", price: PRICE, validators: 0, uptime: 99.9, isFallback: true, lastUpdated: new Date().toISOString() };
export async function GET() {
  try {
    const [validatorRes, listRes] = await Promise.all([
      fetch(`${LCD_URL}/cosmos/staking/v1beta1/validators/${VALIDATOR_OPERATOR}`, { cache: "no-store", signal: AbortSignal.timeout(4000) }),
      fetch(`${LCD_URL}/cosmos/staking/v1beta1/validators?status=BOND_STATUS_BONDED&pagination.limit=500`, { cache: "no-store", signal: AbortSignal.timeout(4000) }),
    ]);
