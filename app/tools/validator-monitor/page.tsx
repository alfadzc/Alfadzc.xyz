    ...
    <main className="min-h-screen bg-white dark:bg-gradient-to-b dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 text-slate-900 dark:text-white">
      ...
      <div className="border-b border-slate-200 dark:border-slate-700/50 bg-white dark:bg-slate-900/50 backdrop-blur-sm">
        <div className="max-w-7xl mx-auto px-6 py-8">
          ...
          <p className="text-slate-600 dark:text-slate-400">Track uptime, performance, and reliability across all validators</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-12 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="rounded-lg border border-slate-200 dark:border-slate-700/50 bg-slate-50 dark:bg-slate-800/60 p-6">
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-2">Total Validators</p>
            <p className="text-3xl font-bold text-emerald-400">{validators.length}</p>
          </div>
          <div className="rounded-lg border border-slate-200 dark:border-slate-700/50 bg-slate-50 dark:bg-slate-800/60 p-6">
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-2">Avg Uptime</p>
          ...
          <div className="rounded-lg border border-slate-200 dark:border-slate-700/50 bg-slate-50 dark:bg-slate-800/60 p-6">
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-2">Last Updated</p>
        ...
        <div className="space-y-4">
          ...
          {loading ? (
            <div className="text-center py-12 text-slate-400">Loading validators...</div>
          ) : validators.length === 0 ? (
            <div className="text-center py-12 text-slate-400">No validators found</div>
          ) : (
                ...
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-400">{validator.chain}</p>
                      </div>
                    </div>
                  ...
                  <div className="space-y-2 border-t border-slate-700/30 pt-4">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-emerald-400">${validator.totalBondedUSD}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-slate-400">Tokens:</span>
                      <span className="text-slate-300">{validator.totalBonded}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-slate-400">Network Validators:</span>
                      <span className="text-slate-300">{validator.validators}</span>
                    </div>
                  </div>
