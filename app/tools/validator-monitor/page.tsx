    ...
    const fetchValidators = async () => {
      try {
        const [
          ...
          fetch(
            `/api/metrics/multi-chain?t=${Date.now()}`,
            { cache: "no-store" }
          )
            .then((r) => r.json())
            .catch(() => null),
        ...
        if (metricsData?.chains) {
          metricsData.chains.forEach((c: any) => {
            if (c.chain) {
              priceMap[c.chain.toLowerCase()] =
                Number(c.price ?? 0);

              tvlMap[c.chain.toLowerCase()] =
                Number(c.totalBondedUSD ?? 0);
            }
          });
        }
        ...
        const validatorsData =
          validatorResults
            ...
            .map((r) => {
               ...
               * NORMALIZE RANK
               *
               * Pastikan rank selalu number.
               * Jika API mengirim string "5", akan menjadi 5.
               * Jika API mengirim null/undefined, menjadi 0.
               */
              const v: ValidatorMetrics = {
                ...raw,
              ...
              if (
                chainKey &&
                priceMap[chainKey] !== undefined
              ) {
                v.price = priceMap[chainKey];
              }

              if (
                chainKey &&
                tvlMap[chainKey] !== undefined
              ) {
                v.totalBondedUSD =
                  tvlMap[chainKey].toFixed(2);
              }

              console.log(
                `✅ Validator ${v.chain} rank:`,
                v.rank
              );
    ...
    <main className="min-h-screen bg-white dark:bg-gradient-to-b dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 text-slate-900 dark:text-white">
      ...
      <div className="border-b border-slate-200 dark:border-slate-700/50 bg-white dark:bg-slate-900/50 backdrop-blur-sm">
        <div className="max-w-7xl mx-auto px-6 py-8">
          ...
          <p className="text-center font-semibold text-slate-900 dark:text-slate-300">
            Track Uptime, Performance and Reliability Across all Validator
          </p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-12 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="rounded-lg border border-blue-500 dark:border-blue-500 bg-slate-50 dark:bg-slate-800/60 p-6 transition-all duration-300 hover:-translate-y-1 hover:border-blue-400 dark:hover:border-blue-400 shadow-none hover:shadow-[0_0_30px_rgba(59,130,246,0.8)] dark:hover:shadow-[0_0_40px_rgba(59,130,246,1)]">
            <p className="text-base font-bold text-center text-slate-800 dark:text-slate-300 mb-2">
              Total Validators
            </p>
            ...
            <div className="flex items-center justify-center py-12">
              <div className="flex flex-col items-center gap-4">
                ...
                <div className="relative w-20 h-20 bg-black rounded-full flex items-center justify-center shadow-inner mb-8">
                  ...
                  <div className="absolute inset-0 rounded-full border-4 border-transparent border-t-[#0088ff] border-l-[#0088ff] border-b-[#0088ff] animate-spin" />

                  <div className="flex space-x-1 z-10 items-center h-4">
                    <div className="w-1.5 h-1.5 bg-white rounded-full dot-wave dot-delay-1" />
                    <div className="w-1.5 h-1.5 bg-white rounded-full dot-wave dot-delay-2" />
                    <div className="w-1.5 h-1.5 bg-white rounded-full dot-wave dot-delay-3" />
                  </div>
                </div>
                ...
                >
                  ...
                  <div className="flex space-x-1 items-center h-5">
                    <span
                      className="text-slate-300 dot-wave dot-delay-1"
                    ...
                    <span
                      className="text-slate-300 dot-wave dot-delay-2"
                      style={{ display: "inline-block" }}
                    >
                      •
                    </span>
                    ...
                    <span
                      className="text-slate-300 dot-wave dot-delay-3"
                      style={{ display: "inline-block" }}
                    >
                      •
                    </span>
                  ...
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col items-center justify-center w-10 h-11 rounded-lg bg-slate-100 dark:bg-slate-700/50 border border-slate-200 dark:border-slate-600 flex-shrink-0">
                        ...
                        <span className="text-[9px] font-bold text-slate-500 dark:text-slate-400 leading-none uppercase tracking-tighter">
                          Rank
                        </span>

                        <span className="text-lg font-extrabold text-[#ff7b00] leading-tight">
                          {Number(validator.rank) > 0
                            ? `#${Number(validator.rank)}`
                            : "—"}
                        </span>
                    ...
                    <div
                      className={`px-3 py-1 rounded-full border text-sm font-semibold ${getUptimeBgColor(
                        validator.uptime
                      )}`}
                    >
                      <span
                        className={getUptimeColor(
                          validator.uptime
                        )}
                      >
                        {(
